...
# Pyrogram's own Client absorbs any FloodWait <= this many seconds by
# sleeping internally and retrying on the SAME account -- silently, without
# raising, so our account-failover logic in _try_each_account never even
# sees it. Default is 10s, long enough that one throttled account can make
# a request look "stuck" instead of failing over to an idle account.
# Kept as a small positive number (not 0) -- sleep_threshold=0/negative has
# behaved inconsistently across pyrogram versions in some builds.
SLEEP_THRESHOLD = 2
...
async def start():
    ...
    for i, session in enumerate(config.SESSION_STRINGS, start=1):
        ...
        try:
            ...
            logger.error(f"Userbot {i}: failed to start ({ex}). Skipping this account.")
...
async def _try_each_account(action, action_name: str):
    ...
    for client in available:
        logger.info(f"{action_name}: trying {client.name}")
        try:
            result = await action(client)
            logger.info(f"{action_name}: served by {client.name}")
            return result
        except FloodWait as e:
            logger.warning(f"{action_name}: {client.name} flood-waited {e.value}s, trying next account.")
    ...
    if last_wait is not None and last_wait <= config.MAX_FLOODWAIT_SECONDS:
        ...
        for client in available:
            logger.info(f"{action_name}: retrying {client.name} after cooldown")
            try:
                result = await action(client)
                logger.info(f"{action_name}: served by {client.name} (after cooldown)")
                return result
            except FloodWait as e:
                logger.warning(f"{action_name}: {client.name} still flood-waited {e.value}s.")
                continue
